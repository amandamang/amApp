...
public class MainActivity extends AppCompatActivity implements SearchView.OnQueryTextListener, ResultsAdapter.ItemClickListener, View.OnKeyListener {
    ...
    public boolean onQueryTextChange(String queryText) {
        text = queryText;
        try{
            new GetSongs().execute(text);

        }catch (Exception ex){
            System.out.println("Error: " + ex.getMessage());
        }
        //rAdapter.filter(queryText);
        return false;
    }
    ...
    public class GetSongs extends AsyncTask<String, String, ResponseResult> {
        String songURL = "https://itunes.apple.com/search?term=" + text + "&entity=allArtist&attribute=allArtistTerm&limit=10";
        ...
        protected ResponseResult doInBackground(String... strings) {
            ...
            try (final CloseableHttpClient httpclient = HttpClients.createDefault()) {
                final HttpGet httpget = new HttpGet(songURL);
    ...
    public void onItemClick(View view, int position) {
        ...
        args.putSerializable("itemClickedList", itemClickedList);
        o.putExtra("BUNDLE", args);*/
        startActivity(o);
    }
